...
if ($my_ip!=$arr_ip2[$i1]) {$array_p[$j1] = $arr_ip2[$i1]; $j1++;}}
/*$array_p = array($arr_ip1, " ", $my_ip);*/
$new_ip = trim(implode(" ", $array_p));
$update = mysql_query ("UPDATE `articles` SET `dislike_ip`='$new_ip' WHERE `id`='$id'");
$upl = $tit-$bit+1;
...
if ($my_ip==$arr_ip1[$i1]) {$arr_ip1[$i1] = '';}}
$new_ip1 = trim(implode(" ", $arr_ip1));
$new_ip2 = $my_a["dislike_ip"]." ".$my_ip;
$update = mysql_query ("UPDATE `articles` SET `dislike_ip`='$new_ip2' WHERE `id`='$id'");
$update = mysql_query ("UPDATE `articles` SET `like_ip`='$new_ip1' WHERE `id`='$id'");
$upl = $tit-$bit-2;
} // if nearly press plus
if ((substr_count($my_a["dislike_ip"],$my_ip)==0)&&
(substr_count($my_a["like_ip"],$my_ip)==0))  {
$new_ip = $my_a["dislike_ip"]." ".$my_ip;
$update = mysql_query ("UPDATE `articles` SET `dislike_ip`='$new_ip' WHERE `id`='$id'");
$upl = $tit-$bit-1;
